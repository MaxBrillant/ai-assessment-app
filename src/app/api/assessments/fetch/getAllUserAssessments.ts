...
export async function getAllUserAssessments() {
  try {
    ...
    if (error) {
      throw new Error("Error while getting all assessments: " + error?.message);
    }
    ...
    throw new Error("Error while getting all user assessments: " + err);
  }
}
